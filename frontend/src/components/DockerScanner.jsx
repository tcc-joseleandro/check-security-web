...
import { Box, Search, Loader2, XCircle, CheckCircle, Lock, User } from 'lucide-react';
  ...
  const handleScan = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/container/scan', {
        image: imageName,
        scanner: scannerType,
        // Enviamos as credenciais (opcionais para Trivy, necessárias para Scout se imagem for privada)
    ...
    <div style={{ padding: '60px 20px', maxWidth: '800px', margin: '0 auto', color: '#f8fafc' }}>
      ...
      <form onSubmit={handleScan} style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginBottom: '40px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <input 
            type="text" 
            placeholder="Ex: node:18-alpine ou username/repo:latest"
            value={imageName}
        ...
        {/* Campos de Autenticação Condicionais para Docker Scout */}
        {scannerType === 'docker-scout' && (
          <div style={{ 
            ...
            <div style={{ gridColumn: '1 / span 2', fontSize: '0.8rem', color: '#38bdf8', marginBottom: '5px', display: 'flex', alignItems: 'center', gap: '5px' }}>
              <Lock size={14} /> Autenticação Docker Hub (Obrigatória para Scout)
            </div>
